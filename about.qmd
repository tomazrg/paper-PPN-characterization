...
unique(number_tillage$tillage)

number_tillage %>% 
  filter(tillage == "No-till")

number_tillage %>% 
  filter(tillage == "Minimum")

number_tillage %>% 
  filter(tillage == "Conventional")
```
...
unique(number_cropping$cropping)

number_cropping %>% 
  filter(cropping == "Corn_soybean")

number_cropping %>% 
  filter(cropping == "Corn_others")
```
...
```{r}
number_soil = population %>% 
  group_by(samples,soil) %>% 
  summarise(
    n = sum(quantity)
  )

unique(number_soil$soil)
...
number_soil %>% 
  filter(soil == "Chernossolo")

number_soil %>% 
  filter(soil == "Gleissolo")

number_soil %>% 
  filter(soil == "Latossolo")

number_soil %>% 
  filter(soil == "Neossolo")
```
...
```{r}
  region_density2 = region_density %>% 
    #filter(!region %in% c("North","Northwest")) %>% 
...
# Supplementary 
  ...
  region_density %>% 
    #filter(!region %in% c("North","Northwest")) %>% 
   dplyr::filter(!genus %in% c("Aphelenchus","Aphelenchoides","Criconematidae","Rotylenchulus",
                       "Pratylenchus","Helicotylenchus")) %>% 
   ggplot(aes(reorder(genus,+mean_density), mean_density))+
  geom_bar(stat = "identity", position = "dodge", fill = "steelblue")+
   theme_minimal()+
...
municipality_density <- population %>%
  filter(quantity > 0) %>%
  group_by(genus, city) %>%
  summarise(
    n = n(),
...
ggplot(aes(city,mean_density)) +
 ...
 ggsave("fig/mean_density_city.png", dpi = 600, bg = "white", width = 10, height = 8)
```
...
ggsave("fig/soil_mean_density.png", dpi = 600, bg = "white", width = 10, height = 6)
```
...
```
```{r}
tillage_graphic = tillage_density %>% 
  filter(genus %in% c("Aphelenchus","Aphelenchoides","Criconematidae","Rotylenchulus",
                  "Pratylenchus","Helicotylenchus")) %>% 
  ggplot(aes(reorder(genus,+mean_density), mean_density,
...
```{r}
altitude_graphic = altitude_density %>% 
  filter(genus %in% c("Aphelenchus","Aphelenchoides","Criconematidae","Rotylenchulus",
                  "Pratylenchus","Helicotylenchus")) %>% 
...
```{r}
library(patchwork)
(soil_graphic/corn_soybean/tillage_graphic)
...
ggplot(aes(city,mean)) +
 ...
 #ggsave("fig/abundance_city2.png", dpi = 600, bg = "white", width = 10, height = 6)
```

```{r}
 ggsave("fig/abundance_city.png", dpi = 600, bg = "white", width = 10, height = 6)
```
...
```{r}
aphelenchus = municipality_density %>% 
  filter(genus == "Aphelenchus") %>% 
group_by(city) %>% 
  summarise( 
...
aphelenchoides = municipality_density %>% 
...
rotylenchulus = municipality_density %>% 
...
helicotylenchus = municipality_density %>% 
...
pratylenchus = municipality_density %>% 
...
criconematidae = municipality_density %>% 
  filter(genus == "Criconematidae") %>% 
group_by(city) %>% 
  summarise( 
    quantity = mean(mean_density))
...
ggsave("fig/groups_goias.png", width = 8, height = 6, dpi = 600, bg = "white")
```
